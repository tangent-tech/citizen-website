...
function DoubleConfirmSubmit(msg1, msg2, action, frmName, field) {
	var check1=confirm(msg1);
	if (check1) {
		var check2=confirm(msg2);
		if (check2) {
...
$(document).ready(function(){
	...
	$(document).on("blur", "input.object_security_level", function(ev) {
		var theInput = $(this);
		
		if (theInput.attr('data-IsPublisher') == '1')
			return;
		else {
			if (theInput.attr('data-original_object_security_level') != theInput.val()) {
				
				
				$( "#SecurityLevelDialog" ).dialog({
					resizable: false,
					height: 115,
					width: 500,
					modal: true,
					buttons: {
						...
						"Reset Security Level": function() {
							theInput.val( theInput.attr('data-original_object_security_level') );
							$( this ).dialog( "close" );
						}
					}
				});
...
*/
	if (MyJS == 'product_tree' || MyJS == 'product_category_tree') {
		$('#PRODUCT_TREE').tree({
			...
			types : {
				...
				"ENABLE_PRODUCT_ROOT" : {
					...
					valid_children : [ 	"ENABLE_PRODUCT_CATEGORY", "DISABLE_PRODUCT_CATEGORY",
										...
										"ENABLE_PRODUCT_GROUP", "DISABLE_PRODUCT_GROUP"]
				},
				...
				"ENABLE_PRODUCT_GROUP" : {
					valid_children : [ 	"ENABLE_PRODUCT", "DISABLE_PRODUCT"],
					icon : {
						image : "../js/jstree/product_group.png"
					}									
				},
				"DISABLE_PRODUCT_GROUP" : {
					valid_children : [ 	"ENABLE_PRODUCT", "DISABLE_PRODUCT"],
					icon : {
						image : "../js/jstree/disable_product_group.png"
					}
				},
				"ENABLE_PRODUCT" : {
					creatable	: false,
					...
					items : {
						...
						duplicate : {
							...
							visible	: function (NODE, TREE_OBJ) {
								if (
									$(NODE).attr('data-object_type') == 'PRODUCT' ||
									$(NODE).attr('data-object_type') == 'PRODUCT_CATEGORY' ||
									$(NODE).attr('data-object_type') == 'PRODUCT_GROUP'
								)
									return true;
								else
									return false;
							...
							action	: function (NODE, TREE_OBJ) {
								var response = $.ajax({
									type:	"POST",
									async:	false,
									url:	"product_tree_duplicate.php",
									dataType:	"xml",
									data:	"object_type=" + $(NODE).attr('data-object_type') +"&link_id=" + $(NODE).attr('data-object_link_id'),
								...
								if (status == 'ok') {
									window.location = 'product_tree.php';
								}
								else {
									$.jGrowl(msg);
								}
						...
						duplicate_with_shared_children : {
							label	: "Duplicate with shared product",
							icon	: "", // you can set this to a classname or a path to an icon like ./myimage.gif
							visible	: function (NODE, TREE_OBJ) {
								if (
									$(NODE).attr('data-object_type') == 'PRODUCT_CATEGORY' ||
							...
							action	: function (NODE, TREE_OBJ) {
								var response = $.ajax({
									type:	"POST",
									async:	false,
									url:	"product_tree_duplicate_with_share_children.php",
									dataType:	"xml",
									data:	"object_type=" + $(NODE).attr('data-object_type') +"&link_id=" + $(NODE).attr('data-object_link_id'),
									error:	function (XMLHttpRequest, textStatus, errorThrown) {
										$.jGrowl("<strong>ERROR:</strong> Not connected to server." + textStatus, {header: GetTime(), sticky: true});
						...
						insert_product : {
							...
							visible	: function (NODE, TREE_OBJ) {
								if ($(NODE).attr('data-object_type') == 'PRODUCT_CATEGORY' || $(NODE).attr('data-object_type') == 'PRODUCT_ROOT' || $(NODE).attr('data-object_type') == 'PRODUCT_GROUP')
									return true;
								else
									return false;
							},
			...
			callback: {
				...
				"ondblclk" : function(NODE, TREE_OBJ) {
					if ($(NODE).attr('data-object_type') == 'PRODUCT_CATEGORY' || $(NODE).attr('data-object_type') == 'PRODUCT_GROUP')
						...
						$(location).attr('href', 'product_root_edit.php?link_id=' + $(NODE).attr('data-object_link_id'));
				}
			}
		});
	}

	else if (MyJS == 'product_tree_special') {
		$('#PRODUCT_TREE').tree({
			lang : {
				new_node : "Untitled"
			},
				...
				contextmenu : {
					items : {
						...
						duplicate : {
							...
							visible	: function (NODE, TREE_OBJ) {
								if (
									$(NODE).attr('data-object_type') == 'FOLDER' ||
									$(NODE).attr('data-object_type') == 'PAGE' ||
									$(NODE).attr('data-object_type') == 'ALBUM' ||
									$(NODE).attr('data-object_type') == 'LINK' ||
									$(NODE).attr('data-object_type') == 'PRODUCT_ROOT_LINK' ||
									$(NODE).attr('data-object_type') == 'NEWS_PAGE' ||
									$(NODE).attr('data-object_type') == 'LAYOUT_NEWS_PAGE'
								)
									return true;
								else
									return false;
							...
							action	: function (NODE, TREE_OBJ) {
								var response = $.ajax({
									type:	"POST",
									async:	false,
									url:	"language_tree_duplicate.php",
									dataType:	"xml",
									data:	"object_type=" + $(NODE).attr('data-object_type') +"&link_id=" + $(NODE).attr('data-object_link_id'),
									error:	function (XMLHttpRequest, textStatus, errorThrown) {
										$.jGrowl("<strong>ERROR:</strong> Not connected to server." + textStatus, {header: GetTime(), sticky: true});
								...
								if (status == 'ok') {
									window.location = 'language_tree.php?id=' + gup('id');
								}
								else {
									$.jGrowl(msg);
								}
						...
						insert_layout_news : {
							label	: TXT_INSERT_LAYOUT_NEWS_PAGE,
							visible	: function (NODE, TREE_OBJ) {
		...
		/*
		var treeEle = $('#SITE_ROOT');

		$('#SITE_ROOT').bind('tree.init', function () {
		    $('li div', treeEle).addTouch();
		});
		*/		
	...
	else if (MyJS == 'siteblock_def_list') {
		$("#TheSubmitButton").click(function(ev){
			ev.preventDefault();
			if ($(":checkbox.DeleteBlockDef:checked").length === 0) {
				$("#FrmEditBlock").submit();
	...
	else if (MyJS == 'discount_preprocess_rule_add') {
		$('#DiscountPreprocessRuleTabs').tabs({cookie: { expires: 365 }});
	}
	else if (MyJS == 'discount_postprocess_rule_add') {
		$('#DiscountPostprocessRuleTabs').tabs({cookie: { expires: 365 }});
	}
	else if (MyJS == 'album_add') {
		$('#AlbumTabs').tabs({cookie: { expires: 365 }});
	}
	else if (MyJS == 'album_edit') {
		$('#AlbumTabs').tabs({cookie: { expires: 365 }});
	}
	else if (MyJS == 'album_root_edit') {
		$('#AlbumRootTabs').tabs({cookie: { expires: 365 }});
	}
	else if (MyJS == 'bonuspoint_root_edit') {
		$('#BonusPointTabs').tabs({cookie: { expires: 365 }});
	}	
	else if (MyJS == 'media_edit') {
		$('#MediaTabs').tabs({cookie: { expires: 365 }});
	}
	else if (MyJS == 'datafile_edit') {
		$('#DatafileTabs').tabs({cookie: { expires: 365 }});
	}
	else if (MyJS == 'user_root_edit') {
		$('#UserRootTabs').tabs({cookie: { expires: 365 }});
	}
	else if (MyJS == 'siteblock_add') {
		$('#SiteBlockTabs').tabs({cookie: { expires: 365 }});
	}
	else if (MyJS == 'siteblock_edit') {
		$('#SiteBlockTabs').tabs({cookie: { expires: 365 }});
	}	
	else if (MyJS == 'member_add') {
		$('#MemberTabs').tabs({cookie: { expires: 365 }});
	}
	else if (MyJS == 'product_add') {
		$('#ProductTabs').tabs({cookie: { expires: 365 }});

		$("table.LeftHeaderTable").on("change", "input.discount_type:checked", function() {
			if ($(this).val() == '4')
				$('.ProductPriceLevelContainer', $(this).closest('table')).show();
			else
				$('.ProductPriceLevelContainer', $(this).closest('table')).hide();
		});
		
		$("input.discount_type").change();

		$(".AddMoreProductPriceLevelLink").click(function() {
			var ExtraRow = $('.ProductPriceLevelInput', $(this).closest('table')).eq(0).clone();
			$("input", ExtraRow).val('');
			ExtraRow.appendTo($(this).closest('table'));
			ExtraRow.show();
		});
		$(".AddMoreProductPriceLevelLink").click();
		$('.ProductPriceLevelContainer').on("click", ".RemoveProductPriceLevelLink", function() {
			if ($(".RemoveProductPriceLevelLink").size() > 1)
				$(this).closest('tr').remove();
		});
	}
	else if (MyJS == 'product_brand_add') {
		$('#ProductBrandTabs').tabs({cookie: { expires: 365 }});
	}
	else if (MyJS == 'product_brand_edit') {
		$('#ProductBrandTabs').tabs({cookie: { expires: 365 }});
	}
	else if (MyJS == 'product_root_link_edit') {
		$('#ProductRootLinkTabs').tabs({cookie: { expires: 365 }});
	}
	else if (MyJS == 'product_root_link_real_edit') {
		$('#ProductRootLinkTabs').tabs({cookie: { expires: 365 }});
	}
	else if (MyJS == 'discount_rule_test') {
		$("#AddMoreDiscountRuleProduct").click(function() {
			var ExtraRow = $('.DiscountRuleTestInput').eq(0).clone();
			ExtraRow.appendTo('#DiscountRuleTestTable');
			ExtraRow.show();
		});

		$(".RemoveDiscountRuleTestLink").live('click', function() {
			$(this).closest('tr').remove();
		});

	}
	else if (MyJS == 'discount_preprocess_rule_edit') {
		$('#DiscountPreprocessRuleTabs').tabs({cookie: { expires: 365 }});

		$("#AddMoreItemConditionLink").click(function() {
			var ExtraRow = $('.ItemConditionInput').eq(0).clone();
			ExtraRow.appendTo('#DiscountPreprocessRuleItemConditionListTable');
			ExtraRow.show();
		});

		$(".RemoveItemConditionLink").live('click', function() {
			$(this).closest('tr').remove();
		});

		$(".ItemConditionTypeIDInput").live('change', function() {
			var TheRow = $(this).closest('tr');
			$('.ItemConditionParameterContainer', TheRow).hide();
			$('.ItemConditionParameterContainer' + $(this).val(), TheRow).show();
		});

		$(".ItemConditionTypeIDInput").trigger('change');

		$("select[name='discount_preprocess_discount_type_id']").live('change', function() {
			$('div.PreprocessRuleDiscountParameterContainer').hide();
			$('div.PreprocessRuleDiscountParameterContainer' + $(this).val()).show();
		});
		$("select[name='discount_preprocess_discount_type_id']").trigger('change');
		
		
		$("#AddMoreItemExceptConditionLink").click(function() {
			var ExtraRow = $('.ItemExceptConditionInput').eq(0).clone();
			ExtraRow.appendTo('#DiscountPreprocessRuleItemExceptConditionListTable');
			ExtraRow.show();
		});

		$(".RemoveItemExceptConditionLink").live('click', function() {
			$(this).closest('tr').remove();
		});

		$(".ItemExceptConditionTypeIDInput").live('change', function() {
			var TheRow = $(this).closest('tr');
			$('.ItemExceptConditionParameterContainer', TheRow).hide();
			$('.ItemExceptConditionParameterContainer' + $(this).val(), TheRow).show();
		});

		$(".ItemExceptConditionTypeIDInput").trigger('change');		
	}
	else if (MyJS == 'discount_postprocess_rule_edit') {
		$('#DiscountPostprocessRuleTabs').tabs({cookie: { expires: 365 }});

		$(".AddMorePostprocessDiscountLevelLink").click(function() {
			var Div = $(this).closest('div.PostprocessDiscountLevelContainer');
			
			var ExtraRow = $('.PostprocessDiscountLevelInput', Div).eq(0).clone();

			var TheInputIndex = parseInt(Div.attr('data-theinputindex'));
			TheInputIndex++;
			Div.attr('data-theinputindex', TheInputIndex);

			$('input', ExtraRow).each(function(index) {
				$(this).attr('name', function(i, a) {
					return a.replace("[]","["+TheInputIndex+"]");
				});
			});

			ExtraRow.appendTo($('table', Div));
			ExtraRow.show();
		});

		$(".AddMorePostprocessDiscountLevelLink").click();
		$(".RemovePostprocessDiscountLevelLink").live('click', function() {
			var table = $(this).closest('table');
			if ($(".RemovePostprocessDiscountLevelLink", table).size() > 1)
				$(this).closest('tr').remove();
		});
	}
	else if (MyJS == 'product_edit') {
		$('#ProductTabs').tabs({cookie: { expires: 365 }});
		...
		$("#AddMoreProductCatLink").click(function() {
			var ExtraRow = $('.ProductCatInput').eq(0).clone();
			$('select', ExtraRow).attr('name', 'product_category_id[]');
			ExtraRow.appendTo('#ProductCatInputContainer > table');
			ExtraRow.show();
		});
		$('#ProductCatInputContainer').on("click", ".RemoveProductCatLink", function() {
			if ($('tr.ProductCatInput').length > 2)
				$(this).closest('tr').remove();
		});
		
		$('input[name="product_parent_type"]:radio').change(function(){
			if (this.value == 'product_category') {
				$("tr.ProductCatRow").show();
				$("tr.ProductGroupRow").hide();
			}
			else if (this.value == 'product_group') {
				$("tr.ProductCatRow").hide();
				$("tr.ProductGroupRow").show();
			}
		});
	...
	else if (MyJS == 'product_category_edit') {
	...
	else if (MyJS == 'product_category_special_edit') {
	...
	else if (MyJS == 'product_brand_list') {
		var oldSibling;
		$("table.SortTable").tableDnD({
			scrollAmount: 30,
			onDragClass: "ui-state-highlight",
	...
	else if (MyJS == 'product_brand_product_list') {
		var oldSibling;
		$("table.SortTable").tableDnD({
			scrollAmount: 30,
			onDragClass: "ui-state-highlight",
	...
	else if (MyJS == 'bonuspoint_add') {
		$('#BonusPointItemTabs').tabs({cookie: { expires: 365 }});
	}

	else if (MyJS == 'member_edit') {
	...
	else if (MyJS == 'folder_edit') {
		$('#FolderTabs').tabs({cookie: { expires: 365 }});
	}

	else if (MyJS == 'news_add') {
		$('#NewsTabs').tabs({cookie: { expires: 365 }});
	}

	else if (MyJS == 'layout_news_add') {
		$('#LayoutNewsTabs').tabs({cookie: { expires: 365 }});
	}

	else if (MyJS == 'news_edit') {
		$('#NewsTabs').tabs({cookie: { expires: 365 }});
	}
	else if (MyJS == 'layout_news_category_edit') {
		$('#LayoutNewsCatTabs').tabs({cookie: { expires: 365 }});
	}
	else if (MyJS == 'layout_news_root_edit') {
		$('#LayoutNewsTabs').tabs({cookie: { expires: 365 }});
	}
	else if (MyJS == 'layout_news_edit') {
		$('#LayoutNewsTabs').tabs({cookie: { expires: 365 }});
	...
	else if (MyJS == 'stock_in_out_cart_details') {
		...
		$("#BtnExport").click(function(ev) {
	 		ev.preventDefault();
	 		$("input[name='myaction']").val('export');
	 		$("#FrmEditBlock").submit();
		});
	}
	else if (MyJS == 'discount_bundle_rule_list') {
		var oldSibling;
		$("table.SortTable").tableDnD({
			scrollAmount: 30,
			onDragClass: "ui-state-highlight",
			...
			onDrop: function(table, row) {
				var response = $.ajax({
					type:	"POST",
					async:	false,
					url:	"discount_bundle_rule_sort.php",
					dataType:	"xml",
					data:	'table_id=' + $(table).attr('id') + '&' + $.tableDnD.serialize(),
					error:	function (XMLHttpRequest, textStatus, errorThrown) {
						$.jGrowl("<strong>ERROR:</strong> Not connected to server." + textStatus, {header: GetTime(), sticky: true});
	...
	else if (MyJS == 'discount_preprocess_rule_list') {
		var oldSibling;
		$("table.SortTable").tableDnD({
			scrollAmount: 30,
			onDragClass: "ui-state-highlight",
			...
			onDrop: function(table, row) {
				var response = $.ajax({
					type:	"POST",
					async:	false,
					url:	"discount_preprocess_rule_sort.php",
					dataType:	"xml",
					data:	'table_id=' + $(table).attr('id') + '&' + $.tableDnD.serialize(),
					error:	function (XMLHttpRequest, textStatus, errorThrown) {
						$.jGrowl("<strong>ERROR:</strong> Not connected to server." + textStatus, {header: GetTime(), sticky: true});
	...
	else if (MyJS == 'discount_postprocess_rule_list') {
		var oldSibling;
		$("table.SortTable").tableDnD({
			scrollAmount: 30,
			onDragClass: "ui-state-highlight",
			...
			onDrop: function(table, row) {
				var response = $.ajax({
					type:	"POST",
					async:	false,
					url:	"discount_postprocess_rule_sort.php",
					dataType:	"xml",
					data:	'table_id=' + $(table).attr('id') + '&' + $.tableDnD.serialize(),
					error:	function (XMLHttpRequest, textStatus, errorThrown) {
						$.jGrowl("<strong>ERROR:</strong> Not connected to server." + textStatus, {header: GetTime(), sticky: true});
	...
	else if (MyJS == 'api_error_msg_default_list') {
		$('#ApiErrorMsgListTabs').tabs({cookie: { expires: 365 }});
	}
	else if (MyJS == 'product_root_edit') {
		$('#ProductRootTabs').tabs({cookie: { expires: 365 }});
	}
	else if (MyJS == 'discount_bundle_rule_add') {
		$('#DiscountBundleRuleTabs').tabs({cookie: { expires: 365 }});
	}
	else if (MyJS == 'discount_bundle_rule_edit') {
		$('#DiscountBundleRuleTabs').tabs({cookie: { expires: 365 }});

		$("#AddMoreItemCostConditionLink").click(function() {
			var ExtraRow = $('.ItemCostConditionInput').eq(0).clone();
			ExtraRow.appendTo('#DiscountBundleRuleItemCostConditionListTable');
			ExtraRow.show();
		});

		$(".RemoveItemCostConditionLink").live('click', function() {
			$(this).closest('tr').remove();
		});

		$(".ItemCostConditionTypeIDInput").live('change', function() {
			var TheRow = $(this).closest('tr');
			$('.ItemCostConditionParameterContainer', TheRow).hide();
			$('.ItemCostConditionParameterContainer' + $(this).val(), TheRow).show();
		});

		$(".ItemCostConditionTypeIDInput").trigger('change');

		$("select[name='discount_bundle_item_cost_condition_type_id']").live('change', function() {
			$('div.ItemCostConditionParameterContainer').hide();
			$('div.ItemCostConditionParameterContainer' + $(this).val()).show();
		});
		$("select[name='discount_bundle_item_cost_condition_type_id']").trigger('change');
		
		
		$("#AddMoreItemFreeConditionLink").click(function() {
			var ExtraRow = $('.ItemFreeConditionInput').eq(0).clone();
			ExtraRow.appendTo('#DiscountBundleRuleItemFreeConditionListTable');
			ExtraRow.show();
		});

		$(".RemoveItemFreeConditionLink").live('click', function() {
			$(this).closest('tr').remove();
		});

		$(".ItemFreeConditionTypeIDInput").live('change', function() {
			var TheRow = $(this).closest('tr');
			$('.ItemFreeConditionParameterContainer', TheRow).hide();
			$('.ItemFreeConditionParameterContainer' + $(this).val(), TheRow).show();
		});

		$(".ItemFreeConditionTypeIDInput").trigger('change');		
	}
