<?php
// parameters:
//	object_id (Allowed Object Type: 'BONUS_POINT_ITEM', 'ALBUM', 'PRODUCT', 'PRODUCT_CATEGORY')
//	object_name
//	security_level
//	archive_date
//	publish_date
...
if ($ObjectLink['site_id'] != $Site['site_id'])
	APIDie(array('desc' => 'Invalid object_id'));
...
if (object::ValidateCreateObjectInTree(array('BONUS_POINT_ITEM', 'ALBUM', 'PRODUCT', 'PRODUCT_CATEGORY'), $ObjectLink, 'inside', $Site['site_id'], false, 'MEDIA') === false)
	APIDie(array('desc' => 'Invalid object type'));
...
$MediaID = media::NewMediaWithObject($TheFile, $Site, $SmallWidth, $SmallHeight, $BigWidth, $BigHeight, $Resize, $WatermarkIDSmall, $WatermarkIDBig, intval($_REQUEST['security_level']));

if ($MediaID !== false && $MediaID != 0) {
	object::NewObjectLink($ObjectLink['object_id'], $MediaID, trim($_REQUEST['object_name']), 0, 'normal', DEFAULT_ORDER_ID);
