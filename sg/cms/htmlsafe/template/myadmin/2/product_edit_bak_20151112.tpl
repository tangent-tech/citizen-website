				...
				<div class="AdminEditDetailsBlock">
					<table class="LeftHeaderTable">
						...
						{if $Site.site_module_bonus_point_enable == 'Y'}
							<tr>
								<th> 積分獎賞 </th>
								<td> <input type="text" name="product_bonus_point_amount" value="{$Product.product_bonus_point_amount}" size="90" maxlength="255" /> </td>
							</tr>
						{/if}
						<tr>
							<th> {$ProductCustomFieldsDef.product_price} </th>
							<td>{$Site.currency_shortname} <input type="text" name="product_price" value="{$Product.product_price}" /></td>
						</tr>
						{if $ProductCustomFieldsDef.product_price2 != ''}
							<tr>
								<th> {$ProductCustomFieldsDef.product_price2} </th>
								<td>{$Site.currency_shortname} <input type="text" name="product_price2" value="{$Product.product_price2}" /></td>
							</tr>
						{/if}
						{if $ProductCustomFieldsDef.product_price3 != ''}
							<tr>
								<th> {$ProductCustomFieldsDef.product_price3} </th>
								<td>{$Site.currency_shortname} <input type="text" name="product_price3" value="{$Product.product_price3}" /></td>
							</tr>
						{/if}
							...
							<tr>
								...
								<td>
									<table>
										<tr>
											<td>
												<input type="radio" name="discount_type" value="0" {if $Product.discount_type == 0}checked="checked"{/if} /> 無折扣 <br />
												<input type="radio" name="discount_type" value="1" {if $Product.discount_type == 1}checked="checked"{/if} /> <input type="text" name="discount1_off_p" value="{$Product.discount1_off_p}" maxlength="2" size="2" />% 扣減 <br />
												<input type="radio" name="discount_type" value="2" {if $Product.discount_type == 2}checked="checked"{/if} /> {$Site.currency_shortname} <input type="text" name="discount2_price" value="{$Product.discount2_price}"  maxlength="5" size="5" /> <input type="text" name="discount2_amount" value="{$Product.discount2_amount}" maxlength="2" size="2" />件 <br />
												<input type="radio" name="discount_type" value="3" {if $Product.discount_type == 3}checked="checked"{/if} /> 買<input type="text" name="discount3_buy_amount" value="{$Product.discount3_buy_amount}" maxlength="2" size="2" />送<input type="text" name="discount3_free_amount" value="{$Product.discount3_free_amount}" maxlength="2" size="2" /><br />
											</td>
											<td>
												<input type="radio" name="discount_type" value="4" {if $Product.discount_type == 4}checked="checked"{/if} />　價格水平
												<div id="ProductPriceLevelContainer">
													<table class="TopHeaderTable AlignLeft">
														<tr class="ui-state-highlight">
															<th class="AlignLeft"><a id="AddMoreProductPriceLevelLink" class="ui-state-default ui-corner-all MyIconButton"><span class="ui-icon ui-icon-plus"></span></a></th>
															<th class="AlignLeft">最少數量</th>
															<th class="AlignLeft">價錢</th>
														</tr>
														<tr>
															<td class="AlignLeft"></td>
															<td>0<input type="hidden" name="product_price_level_min[]" value="0" /></td>
															<td><input type="text" name="product_price_level_price[]" value="{$ProductPriceLevelList[0].product_price_level_price}" /></td>
														</tr>
														{foreach from=$ProductPriceLevelList item=L}
															{if $L.product_price_level_min != 0}
																<tr>
																	...
																	<td><input type="text" name="product_price_level_min[]" value="{$L.product_price_level_min}" /></td>
																	<td><input type="text" name="product_price_level_price[]" value="{$L.product_price_level_price}" /></td>
																</tr>
															{/if}
														{/foreach}
														<tr class="ProductPriceLevelInput Hidden">
															<td class="AlignLeft"><a class="RemoveProductPriceLevelLink ui-state-default ui-corner-all MyIconButton"><span class="ui-icon ui-icon-minus"></span></a></td>
															<td><input type="text" name="product_price_level_min[]" /></td>
															<td><input type="text" name="product_price_level_price[]" /></td>
														</tr>
													</table>
												</div>
											</td>
