...
<h1 class="PageTitle">編輯{$LayoutNews.layout_news_root_name|escape:'html'} &nbsp;
...
{foreach from=$BlockDefs item=D}
	<div class="PageEditLeft InnerContainer ui-widget ui-corner-all AdminEditDetailsBlock">
		...
		<div class="InnerContent ui-widget-content ui-corner-bottom">
				...
				{foreach from=$BlockContents[$D.block_definition_id] item=C}
					<tr id="BC-{$C.block_content_id}" class="{if $C.object_is_enable == 'N'}DisabledRow{/if}">
						...
						<td>
							...
							<a href="layout_news_block_delete.php?layout_news_id={$smarty.request.id}&id={$C.block_content_id}" onclick='return confirm("警告! \n 確定刪除？")' class="ui-state-default ui-corner-all MyButton">
			...
			<a href="layout_news_block_add.php?layout_news_id={$smarty.request.id}&block_def_id={$D.block_definition_id}" class="ui-state-default ui-corner-all MyButton"><span class="ui-icon ui-icon-circle-plus"></span>新增區塊</a>
		</div>
	</div>
{/foreach}
<br class="clearfloat" />
