...
<h1 class="PageTitle">{$Site.site_label_layout_news|ucwords} Root List &nbsp;
	<a href="layout_news_category_list.php?language_id={$smarty.request.language_id}" class="ui-state-default ui-corner-all MyButton"><span class="ui-icon ui-icon-arrowreturnthick-1-e"></span>{$Site.site_label_layout_news|ucwords} Category List</a>
...
<div class="InnerContainer ui-widget ui-corner-all AdminEditDetailsBlock">
	<table class="TopHeaderTable ui-helper-reset">
		<tr class="ui-state-highlight">
			<th width="50">ID</th>
			<th width="300">{$Site.site_label_layout_news|ucwords} Root Name</th>
		...
		{foreach from=$LayoutNewsRoots item=R}
			<tr class="{if $R.object_is_enable == 'N'}DisabledRow{/if}">
				...
				<td class="AlignCenter">
					...
					<a href="layout_news_root_delete.php?id={$R.layout_news_root_id}" onclick="return DoubleConfirm('WARNING!\n All {$Site.site_label_layout_news|ucwords} in this root will also be deleted!\n Are you sure you want to delete?', 'WARNING!\nAre you 100% sure?')" class="ui-state-default ui-corner-all MyButton">
						<span class="ui-icon ui-icon-trash"></span> delete
					</a>
					<a href="layout_news_list.php?id={$R.layout_news_root_id}" class="ui-state-default ui-corner-all MyButton">
						<span class="ui-icon ui-icon-calculator"></span> {$Site.site_label_layout_news|ucwords} list
					</a>
				</td>
			</tr>
		{/foreach}
	</table>
	<a href="layout_news_root_add.php?language_id={$smarty.request.language_id}" class="ui-state-default ui-corner-all MyButton"><span class="ui-icon ui-icon-circle-plus"></span>Add {$Site.site_label_layout_news|ucwords} Root</a>
</div>
<br class="clearfloat" />
