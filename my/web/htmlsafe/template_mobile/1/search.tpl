		...
		<div class="main">
			<main>
				...
				<div class="headingLv1Block noBorder">
					<h1 class="headingLv1">Search Results</h1>
				</div>
					...
					<form id="SearchForm" action="{$BASEURL}/search.php" method="GET" role="search">
						<div class="clearfix">
							<ul class="search_itemList">
								...
								<li class="search_item clearfix">
									<p class="search_inputLabel">Category</p>
									<div class="selectbox">
										<select name="search_type" id="">
											<option value="all" {if $smarty.request.search_type == "all" || $smarty.request.search_type == ""}selected{/if}>All</option>
											<option value="product" {if $smarty.request.search_type == "product"}selected{/if}>Product</option>
											<option value="layout_news" {if $smarty.request.search_type == "layout_news"}selected{/if}>News</option>
											<option value="page"  {if $smarty.request.search_type == "page"}selected{/if}>Page</option>
										</select>
									</div>
								</li>
							</ul>
