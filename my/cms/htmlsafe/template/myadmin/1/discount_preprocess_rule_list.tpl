...
<h1 class="PageTitle">Discount Preprocess List</h1>
<p>
	Note: <br />
		...
		{foreach from=$DiscountPreprocessRuleList item=R}
			<tr id="Rule-{$R.discount_preprocess_rule_id}" class="AlignCenter {if $R.object_is_enable == 'N'}DisabledRow{/if}">
				...
				<td>
					...
					<a href="discount_preprocess_rule_delete.php?id={$R.discount_preprocess_rule_id}" onclick="return DoubleConfirm('WARNING!\n Are you sure you want to delete?', 'WARNING!\nAre you 100% sure?')" class="ui-state-default ui-corner-all MyButton">
