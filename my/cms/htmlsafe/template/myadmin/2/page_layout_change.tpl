...
<div class="InnerContainer ui-widget ui-corner-all">
	<h2 class="ui-helper-reset ui-helper-clearfix ui-widget-header ui-corner-top InnerHeader">更改頁面排版 - {$ObjectLink.object_name|escape:'html'}</h2>
	<form enctype="multipart/form-data" name="FrmEditBlock" id="FrmEditBlock" method="post" action="page_layout_change_act.php">
		<div class="InnerContent ui-widget-content AdminEditDetailsBlock">
			<table id="TablePageLayoutChange" class="TopHeaderTable ui-helper-reset">
				...
				<tr>
					<td>
						{$Page.layout_name|escape:'html'} <br />
						{if $Page.layout_file_id != 0}<a href="{$smarty.const.BASEURL}/getfile.php?id={$Page.layout_file_id}" target="_preview" class="PreviewImage"><img {if $Page.block_image_width < 400}width="{$Page.block_image_width}"{else}width="300"{/if} src="{$smarty.const.BASEURL}/getfile.php?id={$Page.layout_file_id}" /></a>{/if}
					</td>
					<td> => </td>
					<td>
		...
		<div class="ui-widget-header ui-corner-bottom InnerHeader">
			...
			<a href="page_edit.php?link_id={$smarty.request.link_id}" class="ui-state-default ui-corner-all MyButton">
				<span class="ui-icon ui-icon-cancel"></span> 取消
			</a>
		</div>
	</form>
</div>
