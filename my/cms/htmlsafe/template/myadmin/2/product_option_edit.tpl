...
<h1 class="PageTitle">編輯{$Site.site_label_product|ucwords}選項 &nbsp;
...
<div class="InnerContainer ui-widget ui-corner-all AdminEditDetailsBlock">
	...
	<form enctype="multipart/form-data" name="FrmEditBlock" id="FrmEditBlock" method="post" action="product_option_edit_act.php">
		<div class="InnerContent ui-widget-content">
			<div class="AdminEditDetailsBlock">
				<p>
					<strong>狀態:</strong> 
						<input type="radio" name="object_is_enable" value="Y" {if $ProductOption.object_is_enable == 'Y'}checked="checked"{/if}/> 啟用
						<input type="radio" name="object_is_enable" value="N" {if $ProductOption.object_is_enable == 'N'}checked="checked"{/if}/> 停用
					<br />
					<strong>{$Site.site_label_product|ucwords}選項編碼:</strong> <input type="text" name="product_option_code" value="{$ProductOption.product_option_code}"/> <br />
				</p>
				<br />
					...
					{foreach from=$SiteLanguageRoots item=R}
						<tr>
							...
							{section name=foo start=0 loop=$ProductFieldsShow.product_option_show_no|intval}
								...
								<td><input type="text" name="product_option_data_text_{$smarty.section.foo.iteration}[{$R.language_id}]" value='{$ProductOptionDataList[$R.language_id].$myfield}' size="6" /></td>
							{/section}
						</tr>
					{/foreach}
				</table>
