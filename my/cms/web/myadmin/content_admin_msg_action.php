...
if ($_REQUEST['myaction'] == 'reject' ) {
	$query =	"	UPDATE	workflow " .
				...
				"	WHERE	workflow_id = '" . intval($ContentAdminMsg['workflow_id']) . "' ";
	...
	if ($ContentAdminMsg['workflow_type'] == 'SECURITY_LEVEL_UPDATE_REQUEST') {
		...
		workflow::NewContentAdminMsg($AdminInfo['content_admin_id'], $ContentAdminMsg['sender_content_admin_id'], $ContentAdminMsg['workflow_id'], $Msg);
	}
}
	...
	if ($ContentAdminMsg['workflow_type'] == 'SECURITY_LEVEL_UPDATE_REQUEST') {
		...
		$query =	"	UPDATE	object " .
					"	SET		object_security_level = '" . intval($ContentAdminMsg['workflow_para_int_1']) .  "' " .
					"	WHERE	object_id = '" . intval($ContentAdminMsg['object_id']) . "' ";
		$result = ave_mysqli_query($query, __FILE__, __LINE__, true);
...
header( 'Location: content_admin_msg_details.php?id=' . $_REQUEST['id'] . '&SystemMessage=' . urlencode(ADMIN_MSG_UPDATE_SUCCESS));