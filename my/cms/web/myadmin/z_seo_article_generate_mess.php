...
foreach ($Keyword_Used as $T)
	$KeywordTagText = $KeywordTagText . strtolower(trim($T)) . ", ";

$smarty->assign('KeywordTagText', $KeywordTagText);
...
function ExtractTextFromArticle($ArticleText) {
	$Sentence = preg_split("/[\s：；，\-。●,.◆…？”\n]+/u", $ArticleText, -1, PREG_SPLIT_NO_EMPTY);
	$NoOfSentences = rand(5, 10);
	$PickSentence = array_rand($Sentence, $NoOfSentences);
	$Text = '';
	for ($i = 0; $i < $NoOfSentences; $i++) {
		if (strlen(trim($Sentence[$PickSentence[$i]])) <= 4)
			continue;
...
$smarty->display('myadmin/' . $CurrentLang['language_id'] . '/z_seo_article_generate_mess.tpl');